...
def aaa():
    ...
    if city_idname[2] == city_idname[1]:
        sb = [city_idname[3] + str(city_idname[2]) + '市']
    else:
        sb = [city_idname[3] + str(city_idname[2]) + '市' + str(city_idname[1]) + '区']
    sb.append('当前天气：' + get_now['now']['text'] + get_now['now']['temp'] + '°C ' + '体感温度：' + get_now['now'][
        'feelsLike'] + '°C')
    sb.append('空气质量指数：' + str(air_now['aqi']))
    # sb.append(daily())
    # sb.append(' 今日天气：'+ daily()[0]['textDay']+ daily()[0]['tempMin']+ '-'+ daily()[0]['tempMax']+ '°C')
