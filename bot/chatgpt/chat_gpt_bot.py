...
class ChatGPTBot(Bot, OpenAIImage):
    def __init__(self):
        ...
        openai.api_key = "sk-b3a4662352bd49e7a59b240790ee474e"
        openai.api_base = "https://api.deepseek.com"

        self.sessions = SessionManager(ChatGPTSession, model="deepseek-chat")
        self.args = {
            "model": "deepseek-chat",  # 对话模型的名称
            "temperature": conf().get("temperature", 0.9),  # 值在[0,1]之间，越大表示回复越具有不确定性
            # "max_tokens":4096,  # 回复最大的字符数
            "top_p": conf().get("top_p", 1),
            "frequency_penalty": conf().get("frequency_penalty", 0.5),  # [-2,2]之间，该值越大则更倾向于产生不同的内容
            "presence_penalty": conf().get("presence_penalty", 0.5),  # [-2,2]之间，该值越大则更倾向于产生不同的内容
            "request_timeout": conf().get("request_timeout", 2000),  # 请求超时时间，openai接口默认设置为600，对于难问题一般需要较长时间
    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            word_list = ['妈', '操', 'cnm', '逼' , '傻逼'  , 'sb' , '弱智' , '滚' , 'fuck']
            if len(reply_content["content"]) > 0:
                ...
                #     reply_content["content"] = "sp傻逼"
                    ...
                    words_in_sentence = jieba.lcut(query)
                    for word in words_in_sentence:
                        if word in word_list:
                            try:
                                response = requests.get("https://act.jiawei.xin:10086/lib/api/maren.php?catalog=qiu")
                                # common qiu yang
                                if response.status_code == 200:
                                ...
                                print(f"发生异常：{e}")
                            break
                    #reply_content["content"] = "如果你有其他问题或需要帮助，请随时告诉我，我会尽力提供帮助。"
